...
public interface PersonRepository extends CrudRepository<Profile, Integer> {
    ...
    @Query("""
               SELECT new ru.checkdev.auth.dto.ProfileDTO(
               p.id, p.username, p.experience, p.photo.id, p.updated, p.created) 
               FROM profile p
               WHERE p.id IN :userIds
               """)
    List<ProfileDTO> findProfilesByUserIds(@Param("userIds") List<Integer> userIds);
}
